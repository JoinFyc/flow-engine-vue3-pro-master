  ...
  <w-dialog v-if="pcMode" :border="false" closeFree width="600px" @ok="selectOk" :title="title" v-model="visible">
    <div class="picker" v-if="visible">
      <div class="candidate" v-loading="loading">
        <div v-if="type !== 'role'">
          <el-input v-model="search" :disabled="type === 'dept'" @input="searchUser" style="width: 95%" clearable placeholder="搜索人员，支持拼音、姓名" prefix-icon="search"/>
        ...
        <div class="org-items" :style="type === 'role' ? 'height: 350px' : ''">
          ...
          <div v-for="(org, index) in orgs" :key="index" :class="orgItemClass(org)" @click="selectChange(org)">
            <el-checkbox :modelValue="org.selected" @change="selectChange(org)" :disabled="disableDept(org)"></el-checkbox>
            <div v-if="org.type === 'dept'">
              <el-icon><FolderOpened /></el-icon>
              <span class="name">{{ org.name }}</span>
              <span @click.stop="nextNode(org)" :class="`next-dept${org.selected ? '-disable' : ''}`">
                <iconify icon="fluent:organization-28-regular" style="font-size: 16px"/>下级
              </span>
            </div>
            <div v-else-if="org.type === 'user'">
              <w-avatar :size="35" :name="org.name" :status="org.isLeader ? 'leader' : ''" :src="org.avatar"/>
            </div>
            <div style="display: inline-block" v-else>
              <iconify icon="fluent:people-12-regular"/>
              <span class="name">{{ org.name }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="selected">
        ...
        <div class="org-items" style="height: 350px">
          ...
          <div v-for="(org, index) in select" :key="index" :class="orgItemClass(org)">
            <div v-if="org.type === 'dept'">
              <el-icon><FolderOpened /></el-icon>
              <span style="position: static; flex: 1" class="name">{{ org.name }}</span>
            </div>
            <div v-else-if="org.type === 'user'" style="display: flex; align-items: center; width: 100%;">
              <w-avatar :size="35" :name="org.name" :src="org.avatar" />
            </div>
            <div v-else>
              <iconify icon="fluent:people-12-regular"/>
              <span class="name">{{ org.name }}</span>
            </div>
            <el-icon @click="noSelected(index)"><Close/></el-icon>
          </div>
        </div>
      </div>
    </div>
        ...
        <cell class="m-org-item" v-for="org in (showUsers ? searchUsers:nodes)" :key="org.id">
          <template #title>
            <div @click="selectChange(org)">
              <checkbox v-model="org.selected" :disabled="disableDept(org)">
                <w-avatar :name="org.name" :src="org.avatar" v-if="org.type === 'user'"></w-avatar>
          ...
          <template #right-icon v-if="org.type === 'dept'">
            <div @click.stop="nextNode(org)" :class="{'m-org-item-next': true, 'm-org-item-next-disabled': org.selected}">
              <iconify icon="fluent:organization-28-regular" />
              <span> 下级</span>
            </div>
          </template>
        </cell>
...
export default {
  ...
  methods: {
    ...
    selectChange(node) {
      if (this.pcMode ? node.selected : !node.selected) {
        this.checkAll = false
        for (let i = 0; i < this.select.length; i++) {
          if (this.select[i].id === node.id) {
