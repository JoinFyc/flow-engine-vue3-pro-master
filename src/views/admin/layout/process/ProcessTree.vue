...
import Subproc from '@/views/common/process/nodes/SubProcNode.vue'
  ...
  methods: {
    getDomTree(node, gatewayId) {
      ...
      if (Process.isPrimaryNode(node)) {
        //普通业务节点
        let childDoms = this.getDomTree(node.children, gatewayId)
        ...
        let branchItems = node.branchs.map((branchNode) => {
          //处理每个分支内子节点
          branchNode.gatewayId = node.id
          this.toMapping(branchNode)
          let childDoms = this.getDomTree(branchNode.children, node.id)
        ...
        //继续遍历分支后的节点
        let afterChildDoms = this.getDomTree(node.children, gatewayId)
    ...
    rectifyBranchNodeId(node){
      if (Process.isBranchNode(node)){
        if (node.id.indexOf('_begin') < 0){
          ...
          //然后再纠正网关分支节点的parentId
          node.branchs.forEach(bc => bc.parentId = node.id)
          //再纠正网关合流节点id及parentId
          node.children.parentId = node.id
          node.children.id = node.children.id.substring(0, 17) + '_end'
        }else {
          //有前缀的话再矫正空节点
          node.children.id = node.children.id.substring(0, 17) + '_end'
        }
      }else if (Process.isEmptyNode(node) && node.children && node.children.id){
        //矫正空节点后续节点parentId
        node.children.parentId = node.id;
        (node.branchs || []).forEach(bn => bn.parentId = node.id)
      }
    },
    //解码渲染的时候插入dom到同级
    ...
    insertNode(type, parentNode) {
      ...
      switch (type) {
        ...
        case 'DELAY':
          ...
          break
        case 'SUBPROC':
          this.insertSubProcessNode(parentNode);
          break;
        case 'TRIGGER':
    ...
    insertSubProcessNode(parentNode){
      parentNode.children["name"] = "子流程"
      parentNode.children["props"] = this.$deepCopy(DefaultProps.SUB_PROC_PROPS)
    },
    ...
    insertInclusivesNode(parentNode){
      const shortId = parentNode.children.id
