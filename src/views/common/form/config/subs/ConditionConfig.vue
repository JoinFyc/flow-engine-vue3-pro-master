  ...
  methods: {
          ...
          <div class="w-condition-box-compare">
            {group.children.length > 1 ? [
              ...
              <el-tooltip effect="dark" content="添加并列条件" placement="top">
                <el-icon onClick={() => this.addBrother(group)} class="icon"><plus/></el-icon>
              </el-tooltip>
            ]: ''}
          </div>
          {
            isRoot ? <el-tooltip effect="dark" content="添加并列条件" placement="top">
              <el-icon onClick={() => this.addBrother(group)} class="icon root-add"><plus/></el-icon>
            </el-tooltip>:''
          }
            ...
            {(group.children.length > 1 ? group.children.map((cd, i) => {
              ...
              <div class="w-condition-options">
                <el-tooltip effect="dark" content="添加子项条件" placement="top">
                  <el-icon onClick={() => this.addChild(group)} class="icon"><plus/></el-icon>
                </el-tooltip>
                <el-tooltip effect="dark" content="删除本条件" placement="top">
                  <el-icon onClick={() => this.delChild(parent, i)} class="el-icon-delete icon"><delete/></el-icon>
                </el-tooltip>
              </div>
            ])}
          </div>
    ...
    delChild(group, i){
      ...
      if (group.children.length === 1){
        //如果剩下的元素是分支，就把分支网上提一级
        if ((group.children[0].children || []).length > 1){
          group.children = group.children[0].children
          group.logic = group.children[0].logic
        }else {
