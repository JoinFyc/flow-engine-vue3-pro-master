<template>
  <div>
    ...
    <el-form-item label="保留小数">
      <el-input-number controls-position="right" :precision="0" :max="3" :min="0" v-model="modelValue.precision"
                       placeholder="小数位数"/>
      位
    </el-form-item>
    <el-form-item label="公式配置">
      <el-button icon="setting" @click="showPanel">设置公式</el-button>
    ...
    <w-dialog title="配置计算公式" :width="modelValue.isPlus ? '760px':'560px'" v-model="visible" @opened="init" @ok="validExplain">
      ...
      <el-alert style="margin-bottom: 10px" v-if="showError" type="error"
                :description="`😢 计算公式/函数设置有误，无法完成计算，${errorInfo}`" :closable="false"/>
      <div class="calc-content" v-if="!modelValue.isPlus">
        <div class="calc-screen">
          ...
          <div class="calc-option">
            <el-button link icon="back" :disabled="explainTemp.length === 0" @click="backOne">退格</el-button>
      ...
      <div v-else>
        ...
        <div style="margin-top: 10px">
          ...
          <template v-if="formAndTableVars.length > 0">
            <el-tooltip :content="copyOk ? '复制成功√' : '点击复制变量'" placement="top"
                        v-for="item in formAndTableVars" :key="item.symbol">
...
export default {
  ...
  methods: {
    ...
    validExplain() {
      ...
      try {
        if (this.modelValue.isPlus) {
          const js = this.jsCodeTemp.replace(
            /formData.field[0-9]+\[[\d+?]+\]/gi,
            (mc) => {
              return '{}'
            }
          )
