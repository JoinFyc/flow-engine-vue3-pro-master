    ...
    <div :class="{ 'node-body': true, error: showError }">
      ...
      <div class="node-body-main">
        <div class="node-body-main-header">
          <ellipsis class="title" @click.native="enableEdit = true" v-if="!enableEdit" hover-tip :content="config.name ? config.name : '条件' + level">
            <template #pre>
              <el-icon size="15" style="margin-right: 5px"><share/></el-icon>
            </template>
          </ellipsis>
          ...
          <span class="option">
            ...
            <el-icon style="margin-left: 5px" @click.stop="$emit('delNode')"><Close/></el-icon>
          </span>
        </div>
  ...
  methods: {
    ...
    getOrdinaryConditionContent(subCondition) {
      switch (subCondition.compare) {
        case 'IN':
          return `${subCondition.title}为[${String(
            subCondition.value
          ).replaceAll(',', '、')}]中之一`
        case 'B':
        ...
        case 'ABA':
          return `${subCondition.value[0]} ≤ ${subCondition.title} ≤ ${subCondition.value[1]}`
        case '<=':
          return `${subCondition.title} ≤ ${this.getDefault(
        ...
        default:
          return `${subCondition.title}${subCondition.compare}${this.getDefault(
            subCondition.value[0],
            ' ?'
          )}`
      }
