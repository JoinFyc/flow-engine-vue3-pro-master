    ...
    <div v-if="show" :class="{ 'node-body': true, error: showError }">
      <div>
        <div class="node-body-header" :style="{ 'background-color': headerBgc }">
          <el-icon style="margin-right: 5px" size="14" v-if="(headerIcon || '') !== ''">
            <component :is="headerIcon"/>
          </el-icon>
          <ellipsis v-if="!enableEdit" @click.native.stop="enableEdit = true" class="name" hover-tip :content="title"/>
          <el-input v-else autofocus v-model="_title" @blur="enableEdit = false"/>
          <el-icon v-if="!isRoot" size="15" style="float: right" @click="$emit('delNode')"><Close/></el-icon>
        </div>
        <div class="node-body-content" @click="$emit('selected')" >
          <el-icon class="icon" :name="leftIcon" v-if="leftIcon"></el-icon>
...
export default {
  ...
  methods: {},
  emits: ['selected', 'delNode', 'insertNode', 'update:title'],
}
</script>
